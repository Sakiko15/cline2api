...
- Errors: typed error structs (`clineAPIError`, `clineAccountUnavailableError`, `freeModelUnavailableError`) — only `clineAccountUnavailableError` implements `Unwrap`; `clineErrorHTTPStatus` maps `freeModelUnavailableError`→429 and everything else→500, so upstream status codes are not propagated to clients.
