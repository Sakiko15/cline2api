...
默认只监听 `127.0.0.1`（仅本机可访问）。如需局域网内其他设备访问，可用参数或环境变量指定监听 IP：
...
管理后台「代理配置」中会**自动检测并展示本机 IP 列表**，监听 `0.0.0.0` 时局域网设备通过这些地址访问。

> ⚠️ **安全警告**：管理后台 `/admin/` 无鉴权，监听非回环地址（如 `0.0.0.0`）会将其暴露给局域网。
