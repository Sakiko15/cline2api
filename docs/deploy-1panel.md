...
services:
  cline-proxy:
    ...
    environment:
      # 推荐：首启引导管理密码（未设密码时管理 API 仅允许本机访问，见第 6 节）
...
**fail-closed 机制**：未设置管理密码时，管理 API **只允许本机（127.0.0.1）访问**；从公网直接访问 `/admin/api/*` 会返回 403（静态页面 `/admin/` 仍可打开用于展示）。因此公网首次部署有两条引导路径：

- **方式 A（推荐）**：第 4 节编排 YAML 中已配置 `CLINE_ADMIN_PASSWORD` 环境变量 → 容器首次启动即自动设置该密码，直接用 `http://<VPS_IP>:3457/admin/` 以此密码登录
- **方式 B**：不设环境变量，通过 SSH 隧道进入本机设置：`ssh -L 3457:127.0.0.1:3457 <VPS>`，然后在本地浏览器打开 `http://127.0.0.1:3457/admin/` 设置密码

已设密码后，公网访问恢复正常（输入密码登录）。
